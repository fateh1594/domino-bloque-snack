...
export function HandArea({ myHand, onSelect, selectedIdx, isMyTurn }) {
  return (
    <View style={styles.container}>
      <Text style={styles.status}>{isMyTurn ? "À VOUS" : "ATTENTE..."}</Text>
        ...
        {myHand.map((p, i) => (
          ...
          >
            <DominoFace 
              ...
              w={HDW} h={HDH} 
              vertical={true} 
              borderColor={selectedIdx === i ? C.gold : '#444'}
              borderWidth={selectedIdx === i ? 3 : 1}
            />
          </TouchableOpacity>
        ))}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  status: { color: C.gold, textAlign: 'center', fontSize: 10, fontWeight: 'bold', marginVertical: 5 },
