...
export const C = { domino: '#fdfcf7', dot: '#1a1a1a', gold: '#c9a84c' };
export const HDW = Math.floor(width / 8); 
export const HDH = HDW * 2;

const DOTS = {
  1: [{x:50, y:50}], 2: [{x:25, y:25}, {x:75, y:75}], 
...
export const DominoFace = ({ a, b, w, h, vertical, borderColor = '#333' }) => {
  ...
  return (
    <View style={[S.card, { width: w, height: h, borderColor, flexDirection: vertical ? 'column' : 'row' }]}>
      {renderHalf(a)}
...
const S = StyleSheet.create({
  card: { backgroundColor: C.domino, borderWidth: 1.5, borderRadius: 4, elevation: 3, overflow: 'hidden' },
